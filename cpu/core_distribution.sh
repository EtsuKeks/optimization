...
# Анализируем нагрузку процесса в данную секунду с разбивкой по ядрам CPU. Используется утилита pidstat,
# которая мониторит все потоки процесса и их активность. Скрипт суммирует %CPU всех потоков,
# сгруппированных по ID ядра, на котором они исполнялись. Тут важное замечание: pidstat делает
...
# Ставим локаль, чтобы числа точно были с точкой (10.5), а не с запятой - иначе сложение снизу сломается
export LC_NUMERIC=C
